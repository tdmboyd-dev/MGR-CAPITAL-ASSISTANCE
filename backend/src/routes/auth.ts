...
// =============================================================================
// DEBUG LOGIN — Detailed error logging (TEMPORARY)
// =============================================================================
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      steps.push(`   - password: ${password ? "[provided]" : "[missing]"}`);
      steps.push(`   - req.ip: ${req.ip}`);

      if (!email || !password) {
        return res.json({ success: false, error: "Email and password required", steps });
      }
      ...
      steps.push(`3. Identifier: ${identifier}`);

      const lockoutStatus = isLockedOut(identifier);
      steps.push(`4. Lockout check: ${JSON.stringify(lockoutStatus)}`);

      if (lockoutStatus.locked) {
        return res.json({ success: false, error: "Account locked", steps });
      }

      steps.push("5. Calling authService.login...");
      ...
      steps.push(`6. Login result: success=${result.success}, error=${result.error || "none"}`);

      if (!result.success || !result.tokens || !result.user) {
        return res.json({ success: false, error: result.error || "Login failed", steps });
      }

      steps.push("7. Login successful");
      ...
      res.json({ success: false, error: error.message, steps, stack: error.stack });
    }
  })
);
